  ...
  const performSearch = () => {
    ...
    fetch("index.php?c=bus&m=searchBuses", {
      ...
      .then((data) => {
        ...
        <td>${bus.hasWiFi === 1 ? "Sí" : "No"}</td>
        <td>${bus.hasAC === 1 ? "Sí" : "No"}</td>
        <td>${bus.hasToilet === 1 ? "Sí" : "No"}</td>
        <td>
          <a href="?c=bus&m=deleteBus&id=${
            bus.idBus
          }" class="delete-button" onclick="return confirm('¿Estás seguro de eliminar el bus de matrícula ${
            bus.idBus
          }?')">
            <span class="material-symbols-outlined">delete</span>
          </a>
          <a  href="?c=company&m=mainProfile_busesEdit&id=${
            bus.idBus
          }" class="delete-button" >
