...
class Admin_Controller extends Controlador
{
  ...
  public function searchRequests()
  {
    $data = [
      'msg' => '',
      'status' => '',
      'companyRequests' => [] // Inicializa la matriz de resultados de solicitudes de empresa
    ];

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $searchTerm = $_POST['searchTerm']; // Obtén el término de búsqueda del formulario

      // Realiza la búsqueda de solicitudes de empresa en el modelo
      $requestModel = new Petition("", "", "", "", ""); // Reemplaza 'RequestModel' con el nombre real de tu modelo de solicitud
      $data['companyRequests'] = $requestModel->searchCompanyRequests($searchTerm);

      if (empty($data['companyRequests'])) {
